...
import { ColorTokens, font, lightColor } from './tokens';
...
/**
 * 28 of the styles below name a colour, so the scale cannot be a module-level
 * constant once there are two palettes: it is built per palette instead, and
 * handed out already built by `useTheme()`. Both palettes are built once at
 * module load, so a component's `type` reference is stable and nothing has to
 * memoise it.
 */
export const makeType = (color: ColorTokens) =>
  ({
...
/**
 * The light-palette scale, for tests and anything outside the render tree.
 * A screen must take its scale from `useTheme()`; importing this one is how a
 * screen silently stays light.
 */
export const type = makeType(lightColor);
...
export const monoBlockFor = (color: ColorTokens): TextStyle => ({
  fontFamily: font.mono,
  fontSize: 15.5,
...
export const monoGivenFor = (color: ColorTokens): TextStyle => ({
  fontFamily: font.mono,
  fontSize: 13.5,
...
export const monoBlock = monoBlockFor(lightColor);
export const monoGiven = monoGivenFor(lightColor);
